...
controller.login = async function (req, res) {
  try {
    ...
    // Se o usuário não foi encontrado, retorna HTTP 401: Unauthorized
    if(!user) return res.send(401).end()

    // Usuário encontrado: conferimos a senha
    const passwordOk = await bcrypt.compare(req.body.password, user.password)

    // Senha incorreta: retorna HTTP 401: Unauthorized
    if(!passwordOk) return res.status(401).end()
    ...
    // Retorna o HTTP 200: OK, com o token no corpo da resposta
    res.send({ token })
